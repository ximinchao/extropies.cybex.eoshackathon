...
// 	if (sizeof(pstHeader->region) > (nDataLen - iOffset))
// 	{
// 		iRtn = -1;
// 		goto END;
// 	}
// 	pstHeader->region = 0;
// 	for (i = 0; i < sizeof(pstHeader->region); i++)
// 	{
// 		pstHeader->region |= ((uint16_t)pbData[iOffset + i]) << (i * 8);
// 	}
// 	iOffset += sizeof(pstHeader->region);
...
// 	if (pbData)
// 	{
// 		if ((*pnDataLen - iOffset) < sizeof(pstHeader->region))
// 		{
// 			iRtn = -1;
// 			goto END;
// 		}
// 		for (i = 0; i < sizeof(pstHeader->region); i++)
// 		{
// 			pbData[iOffset + i] = (unsigned char)((pstHeader->region >> (i * 8)) & iFilterMask);
// 		}
// 	}
// 	iOffset += sizeof(pstHeader->region);
...
int eos_util_tx_header_from_json(const cJSON * const pJson, eos_transaction_header * const pstHeader)
{
	...
	// 	uint16_t			region; ///< the computational memory region this transaction applies to.
// 	pNode = cJSON_GetObjectItem(pJson, "region");
// 	if (!pNode)
// 	{
// 		iRtn = -1;
// 		goto END;
...
// 		goto END;
// 	}
// 	pstHeader->region = pNode->valueint;
